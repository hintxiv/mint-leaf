...
import { buffDetailsToStatus, getStoredCustomAction } from './customActionsStore'
...
export const readPreferences = (): Preferences => {
    try {
        ...
        if (value?.version === 1 && value.actions && value.sharedGcds) return value
...
export const dataActionToDefaultAction = (
    ...
    const group = matchingGcdGroup(job, definition)
    ...
    const common = {
        id: data.id,
        name: data.name ?? definition?.names[locale] ?? legacy?.name ?? '',
    ...
    if ((overrides.type ?? definition?.kind ?? 'gcd') === 'ogcd') {
    ...
    return {
        ...
        recastTime: overrides.recastTime ?? (group ? preferences.sharedGcds[group] : undefined)
            ?? (definition?.baseGcdRecastMs !== undefined ? definition.baseGcdRecastMs / 1000 : 2.5),
        castTime: overrides.castTime ?? (definition?.baseCastTimeMs !== undefined ? definition.baseCastTimeMs / 1000 : 0),
    }
}
...
export const applySharedRecast = (actions: Action[], group: string, value: number): Action[] =>
    actions.map(action => isSharingRecast(action) && action.defaults?.gcdGroup === group
        ? { ...action, recastTime: value } : action)
...
export const saveActionEdit = (previous: Action, next: Action, job: string, locale: Locale, edit: ActionEdit = 'edit') => {
    ...
    const identity = { id: next.id, name: next.name, icon: next.imageSrc ? new URL(next.imageSrc, 'http://localhost') : null }
    if (edit === 'reset' || edit === 'reset-recast') {
        ...
        for (const field of ['type', 'castTime', 'lateWeave', 'statusesApplied'] as const) {
    ...
    if (shared) preferences.sharedGcds[shared.group] = shared.value
